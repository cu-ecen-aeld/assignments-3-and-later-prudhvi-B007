...
#include "aesdchar.h"
#include "aesd_ioctl.h"   /* Assignment 9: ioctl interface */

int aesd_major =   0; // use dynamic major
int aesd_minor =   0;

MODULE_AUTHOR("Prudhvi Belide");
MODULE_LICENSE("Dual BSD/GPL");

struct aesd_dev aesd_device;

/**
 * @brief Helper to push a new complete command into circular buffer
 * Frees the oldest entry if buffer already full.
 */
...
/* @return number of valid entries in circ buffer */
...
/* @return total number of bytes across all valid entries */
...
/* Translate (cmd_index, cmd_offset) to absolute file position (f_pos) */
...
static loff_t aesd_llseek(struct file *filp, loff_t offset, int whence)
{
    ...
    if (newpos > endpos) {
        /* clamp at end (or return -EINVAL; either is acceptable—using clamp) */
        newpos = endpos;
    }
...
/* Some 6.x kernels require compat passthrough for 32-bit ioctls */
#ifdef CONFIG_COMPAT
...
struct file_operations aesd_fops = {
    ...
    .llseek         = aesd_llseek,          /* <-- Assignment 9 */
    .unlocked_ioctl = aesd_unlocked_ioctl,  /* <-- Assignment 9 */
#ifdef CONFIG_COMPAT
    .compat_ioctl   = aesd_compat_ioctl,    /* helpful on some kernels */
#endif
};
